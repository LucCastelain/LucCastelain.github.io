...
export default function NavbarComponent({language}: Readonly<NavbarComponentProps>): ReactElement {
    ...
    const closeDropdown = useCallback((e: MouseEvent): void => {
        if(openMenu && !menuRef.current?.contains(e.target as Node) && !menuButtonRef.current?.contains(e.target as Node)) {
            setOpenMenu(false);
        }
    }, [openMenu]);
    ...
    return (
        <nav className={twMerge("fixed top-0 z-50 w-full h-fit bg-white border-gray-200 dark:border-neutral-800 dark:bg-neutral-900 border-b md:shadow-none", openMenu && "shadow-md")}>
            <div className="max-w-screen-xl flex flex-col md:flex-row items-center justify-between mx-auto p-2">
                <div className="flex flex-row w-full md:w-fit">
                    ...
                    <button ref={menuButtonRef}
                            className="ml-auto p-2 md:hidden md:pointer-events-none rounded-lg hover:bg-neutral-200 dark:text-gray-100 dark:hover:bg-neutral-800"
                ...
                <div ref={menuRef}
                     className={twMerge("w-full flex flex-col md:flex-row origin-top transition-all h-0 scale-y-0 md:h-fit md:scale-y-100", openMenu && "h-fit scale-y-100")}>
                    ...
                    <div className="flex flex-row mx-auto mt-2 md:mx-0 md:mt-0">
                        ...
                        <LanguageDropdownComponent language={language}/>
                    </div>
                </div>
            </div>
        </nav>
